    ...
    LazyColumn(contentPadding = PaddingValues(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
        ...
        item {
            AppCard {
                ...
                Text("Tocca un pasto per modificarlo. La cena è facoltativa.", color = MaterialTheme.colorScheme.onSurfaceVariant)
            }
        }

        items(meals, key = { it.id }) { meal ->
            AppCard(modifier = Modifier.clickable { editingMeal = meal }) {
                Row(verticalAlignment = Alignment.CenterVertically) {
                    Column(Modifier.weight(1f)) {
                        ...
                        Text(
                            if (meal.reminder.enabled) "🔔 ${reminderLabel(meal.reminder)}" else "🔕 Promemoria disattivato",
        ...
        item {
            AppCard {
                ...
                if (workouts.isEmpty()) {
                    ...
                    workouts.forEach {
                        Text("${categoryEmoji(it.type)} ${it.time} · ${it.title}", fontWeight = FontWeight.Bold)
                    }
                }
            }
        }
        ...
        item {
            AppCard {
                ...
                else events.forEach { Text("${it.time} · ${it.title}") }
            }
        }
    }
}
    ...
    LazyColumn(contentPadding = PaddingValues(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
        ...
        item {
            AppCard {
                ...
                Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                    listOf("Lun","Mar","Mer","Gio","Ven","Sab","Dom").forEachIndexed { i, d ->
                        FilterChip(selected = day == i, onClick = { day = i }, label = { Text(d) })
                    }
                }
            }
        }
        ...
        if (!(date.dayOfWeek == DayOfWeek.SUNDAY && sundayFree)) {
            items(meals, key = { it.id }) { m ->
                AppCard(modifier = Modifier.clickable { editingMeal = m }) {
                    Text(m.name, fontWeight = FontWeight.Bold)
                    Text("${m.time} · ${m.description}", color = MaterialTheme.colorScheme.onSurfaceVariant)
                    Text(
                        if (m.reminder.enabled) "🔔 ${reminderLabel(m.reminder)}" else "🔕 Promemoria disattivato",
                        color = if (m.reminder.enabled) MaterialTheme.colorScheme.primary else MaterialTheme.colorScheme.onSurfaceVariant,
                        fontSize = 12.sp
                    )
...
@Composable
fun ProfileScreen(prefs: SharedPreferences, onBack: () -> Unit) {
...
fun SimpleInfoScreen(name: String, onBack: () -> Unit) {
    ...
    LazyColumn(contentPadding = PaddingValues(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
        ...
        item {
            AppCard {
                Text("Sezione pronta per essere ampliata.", color = MaterialTheme.colorScheme.onSurfaceVariant)
                Text("🔔 Ogni elemento programmabile può avere un promemoria individuale con anticipo libero.", color = MaterialTheme.colorScheme.primary, fontSize = 12.sp)
            }
        }
    }
}
